...
class ExcelAnalyzer:
    def __init__(self):
        ...
        self.selected_verif = []  # ubah jadi list untuk multiple selection
        self.top_n = 10
    ...
    def load_and_concat_sheets(self, files, sheet_name):
        ...
        if not dfs:
            return None
        combined_df = pd.concat(dfs, ignore_index=True, sort=True)  # sort=True untuk kolom berbeda
        return combined_df
    ...
    def filter_and_group(self):
        # Jika belum ada df, kolom, atau pilihan verifikasi kosong, return kosong
        if self.df is None or self.category_col is None or not self.selected_verif:
            return pd.DataFrame()
        if self.verif_col not in self.df.columns:
            return pd.DataFrame()

        # Filter df berdasarkan kolom verif_col yang ada di list selected_verif (multiple)
        filtered = self.df[self.df[self.verif_col].isin(self.selected_verif)]

        # Hitung value_counts berdasarkan category_col dari data yang sudah difilter
    ...
    def plot_bar(self, grouped):
        ...
        # Buat judul dengan list kondisi yang dipilih
...
def main():
    st.title("📊 Excel Analyzer - Gabungkan Multi File & Sheet dengan Multi Verifikasi")
    ...
    if uploaded_files:
        ...
        # Ganti selectbox jadi multiselect untuk multiple ceklist
        analyzer.selected_verif = st.sidebar.multiselect(
            f"Pilih satu atau beberapa kondisi '{analyzer.verif_col}'",
            options=verif_options,
            default=verif_options[:1]  # default pilih satu yang pertama
        )
